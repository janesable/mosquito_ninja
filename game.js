...
class Game {
    constructor() {
        // Отключаем вертикальные свайпы через Telegram SDK
        if (window.Telegram && window.Telegram.WebApp) {
            Telegram.WebApp.postEvent("web_app_setup_swipe_behavior", {
                allow_vertical_swipe: false,
            });
            console.log("Вертикальные свайпы отключены через Telegram WebApp");
        } else {
            console.warn("Telegram WebApp не обнаружен, возможно запуск в обычном браузере.");
        }
